...
class Agent(nn.Module):
    def __init__(self, env, device, buffer_size=BUFFER_SIZE, reward_scale=REWARD_SCALE, batch_size=256, gamma=0.99, lr_actor=LR_ACTOR, lr_critic=LR_CRITIC, print_period=20, write_mode=True, save_period=12000000):
        ...
        self.memory = ReplayBuffer(self.state_dim, self.action_dim, buffer_size, device=self.device)
        self.act_dim = self.state_dim//2
        self.sta_dim = self.state_dim

        self.actor = Actor(self.sta_dim, self.action_dim, self.device, self.action_bound, lr=self.lr_actor).to(
            self.device)

        self.local_critic_1 = Critic(self.sta_dim, self.act_dim, self.device, lr=self.lr_critic).to(self.device)
        self.local_critic_2 = Critic(self.sta_dim, self.act_dim, self.device, lr=self.lr_critic).to(self.device)
        self.target_critic_1 = Critic(self.sta_dim, self.act_dim, self.device, lr=self.lr_critic).to(self.device)
        self.target_critic_2 = Critic(self.sta_dim, self.act_dim, self.device, lr=self.lr_critic).to(self.device)
        ...
        self.weight_c = torch.tensor(0.0) + \
                        torch.tensor(1.0) * torch.randn(self.action_dim, self.act_dim).to(self.device)
        self.weight_d = torch.tensor(1.0) * torch.randn(1, self.act_dim).to(self.device)
    ...
    def save_model(self):
        save_path = self.save_model_path + 'humanoid_v2_sac_{}.pth'.format(str(self.total_step))   
        torch.save(self.state_dict(), save_path)

    def load_model(self, path=None):
        if path is None:
            self.load_state_dict(torch.load(self.save_model_path + 'humanoid_v2_sac.pth'))
        else:
    ...
    def train(self, max_episode_num=1000, max_time=1000):
        ...
        for episode_idx in range(max_episode_num):
            ...
            for t in range(1, max_time+1):   
                ...
                #next_state, reward, done = np.array(next_state), np.array(reward), np.array(done)
                episode_reward += reward

                # Ignore the "done" signal if it comes from hitting the time horizon.
                masked_done = False if t == self.max_episode_time else done

                self.memory.add(np.hstack([state, action, reward, next_state, 1 if masked_done else 0]))
                self.memory.add_done(np.hstack([state, action, reward, next_state, 1 if masked_done else 0]))
                if done:
                    if t_flag > self.batch_size:
                        self.memory.done_flag = False
                    else:
                        t_flag += 1
                state = next_state
                ...
                for _ in range(NUM_LEARN):
                    ...
                    act_tran = self.ActEncode(actions)

                    # Compute targets for the Q functions
                    with torch.no_grad():
                        sampled_next_actions, next_log_probs = self.actor.get_action_log_prob(next_states)
                        # encode
                        sam_act_nex_tran = self.ActEncode(sampled_next_actions)

                        Q_target_1 = self.target_critic_1.forward(next_states, sam_act_nex_tran).detach()
                        Q_target_2 = self.target_critic_2.forward(next_states, sam_act_nex_tran).detach()
                        y = rewards + self.gamma * (1 - dones) * (
                                torch.min(Q_target_1, Q_target_2) - self.alpha * next_log_probs)

                    # Update Q-functions by one step of gradient descent
                    Q_1_current_value = self.local_critic_1.forward(states, act_tran)
                    # test1 = torch.mean((q1_inv - Q_1_current_value) ** 2)
                    Q_loss_1 = torch.mean((y - Q_1_current_value) ** 2)
                    Q_2_current_value = self.local_critic_2.forward(states, act_tran)
                    Q_loss_2 = torch.mean((y - Q_2_current_value) ** 2)
                    Q_loss = Q_loss_1 + Q_loss_2
                    self.critic_optimizer.zero_grad()
                    Q_loss.backward()
                    self.critic_optimizer.step()
                    temp_critic_loss_list.append((Q_loss_1 + Q_loss_2).item())

                    # Update policy by one step of gradient ascent
                    sampled_actions, log_probs = self.actor.get_action_log_prob(states)
                    sam_act_tran = self.ActEncode(sampled_actions)
                    Q_min = torch.max(self.local_critic_1.forward(states, sam_act_tran),
                                      self.local_critic_2.forward(states, sam_act_tran))
    ...
    def ActEncode(self, x):
        # Linear Transform
        y = torch.mm(x, self.weight_c) + self.weight_d
        # y = torch.tanh(y)
        return y